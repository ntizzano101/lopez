...
alert('eer');
	...
	function confirma(){
		...
		$('input[name="id[]"').each(function()
			{id.push($(this).val()); });		
		$.post("/lopez/proveedores/guardapago",
		{o:original,c:cancelar,i:id,obs:$('#obs').val(),fecha:$('#fecha').val()}, 
		function(data){
			//alert(data);
...
<div class="alert alert-danger  alert-dismissible fade in" id="error" style="display:none">
	...
	<div class="row" id="row2" style="display:none">
		<div class="col-md-12">
				...
				<a href="/lopez/proveedores/listado" class="btn btn-primary">Seguir</a>
				...
				<table class="table">
				  ...
				  <tbody>
					...
					foreach($cc as $c){ 
							...
							<td align="right"><?php echo $fx->moneda(round($c->debe-$c->haber,2))?></td>
							<td  align="right"><?php   echo $fx->moneda($c->cobrado);   ?></td>
							<td  align="right"><?php if($c->cobrado - $c->debe  >= 0){ echo $fx->moneda($c->cobrado - $c->debe );}   ?></td>
							<td align="right">
			<input type="hidden" name="id[]" value="<?=$c->id?>">				
			<input type="hidden" name="original[]" value="<?php echo round($c->debe-$c->haber,2)?>">
				<input type="text" name="cancelar[]" value="<?php 
						if($c->cobrado >0) 
								{  
									if($saldi > $c->cobrado )
											echo $c->cobrado;
